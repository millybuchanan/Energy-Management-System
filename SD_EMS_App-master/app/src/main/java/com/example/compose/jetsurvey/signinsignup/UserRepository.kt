...
sealed class User {
    @Immutable
    data class LoggedInNewUser(val email: String) : User()

    data class LoggedInOldUser(val email: String) : User()
...
object UserRepository {
            ...
            Amplify.DataStore.query(
                ...
                { items ->
                    val signedIn = if (items.hasNext()) {
                        val item = items.next()
                        _user = if (item.newUser) {
                            User.LoggedInNewUser(email)
                        } else {
                            User.LoggedInOldUser(email)
                        }
    ...
    suspend fun setUserToOldUser() {
        when (val currentUser = _user) {
            is User.LoggedInNewUser -> {
                _user = User.LoggedInOldUser(currentUser.email)
                ...
                try {
                    Amplify.DataStore.query(
                        ...
                        { items ->
                            if (items.hasNext()) {
                                val item = items.next()
                                val updatedItem = item.copyOfBuilder().newUser(false).build()

                                CoroutineScope(Dispatchers.IO).launch {
                                    val result = saveToDataStore(updatedItem)
                                    if (result) {
                                        Log.i("UserRepository", "User updated successfully")
                                    } else {
                                        Log.e("UserRepository", "Failed to update user")
                                    }
                                }
                            }
                        },
                        { failure ->
                            Log.e("UserRepository", "Could not query DataStore", failure)
                        }
                    )
                } catch (exception: Exception) {
                    Log.e("UserRepository", "Error querying item from DataStore", exception)
                ...
                // No action needed for other user types
            }
        }
    }
    ...
    fun signUp(email: String, password: String, onSignUpComplete: (success: Boolean) -> Unit) {
        ...
        CoroutineScope(Dispatchers.IO).launch {
            ...
            _user = User.LoggedInNewUser(email)
